...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            potential_action = PgNode_a(action)
            # Set iff all prerequisite literals for the action hold in S0.  This can be accomplished by testing
            # to see if a proposed PgNode_a has prenodes that are a subset of the previous S level.
            prev_S_level = self.s_levels[level]
            if potential_action.prenodes.issubset(prev_S_level):
                good_action = potential_action # prerequisite literals for the action hold in S0
                A_level_actions.append(good_action) 
                ...
                for prev_literal in prev_S_level:
                    prev_literal.children.add(good_action) # connect previous literal to action
                    good_action.parents.add(prev_literal)  # connect back from action to previous literal
        
        # Add A nodes to the current level
    ...
    def add_literal_level(self, level):
        ...
        S_level_literals = set()
        for prev_action in self.a_levels[level-1]:
            for new_literal in prev_action.effnodes: # get the effects produced by the previous actions
                S_level_literals.add(new_literal) # add the effects
                prev_action.children.add(new_literal) # connect from previous action to new literal
                new_literal.parents.add(prev_action) # connect back from new literal to previous action
        
        self.s_levels.append(S_level_literals)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Test to see if B removes something that A adds
        A_adds = set(node_a1.action.effect_add)
        B_removes = set(node_a2.action.effect_rem)
        inconsistent_1 = bool(A_adds & B_removes)
        
        # Test to see if A removes something that B adds
        A_removes = set(node_a1.action.effect_rem)
        B_adds = set(node_a2.action.effect_add)
        inconsistent_2 = bool(B_adds & A_removes)
        
        return inconsistent_1 or inconsistent_2
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Test to see if B removes a positive precondition that A needs
        A_needs = set(node_a1.action.precond_pos)
        B_removes = set(node_a2.action.effect_rem)
        interference_1 = bool(A_needs & B_removes)

        # Test to see if B adds a negative precondition that A doesn't like
        A_hates = set(node_a1.action.precond_neg)
        B_adds = set(node_a2.action.effect_add)
        interference_2 = bool(A_hates & B_adds)

        # Test to see if A removes a positive precondition that B needs
        A_removes = set(node_a1.action.effect_rem)
        B_needs = set(node_a2.action.precond_pos)
        interference_3 = bool(A_removes & B_needs)

        # Test to see if A adds a negative precondition that B doesn't like
        A_adds = set(node_a1.action.effect_add)
        B_hates = set(node_a2.action.precond_neg)
        interference_4 = bool(A_adds & B_hates)

        return interference_1 or interference_2 or interference_3 or interference_4
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # can the parents of both exist at the same time?
        for parent_A in node_a1.parents:
            for parent_B in node_a2.parents:
                if parent_A.is_mutex(parent_B): # not going to work
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        for action_created_S1 in node_s1.parents:
            for action_created_S2 in node_s2.parents:
                if not action_created_S1.is_mutex(action_created_S2): # found a pair that is not inconsistent
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        level = 0
        goals_remaining = set(self.problem.goal)
        for s_level in self.s_levels:
            for literal in s_level:
                if literal in goals_remaining:
                    goals_remaining.remove(goal)
                    level_sum += level
            if bool(goals_remaining):
                level += 1 # keep looking
            else:
                break # all goals acheived
        return max(1, level_sum)